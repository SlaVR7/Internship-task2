  ...
  return (
    <Flex className={'grow bg-gray-300 dark:bg-grayMColor'}>
      <Flex vertical className={'max-w-[1440px] mx-auto px-[100px] py-[50px] w-[100%]'}>
        {contextHolder}
        <Row className={'text-accentColor dark:text-accentDarkColor text-[36px] font-bold pb-[48px]'}>Please, check your order details:</Row>
        <CartList isPromoCodeActive={isPromoCodeActive} isOrderPage={true} />
        <Flex justify={'space-between'} className={'mt-[30px]'}>
          <Row className={'text-accentColor dark:text-accentDarkColor text-[24px] font-bold pb-[48px]'}>Do you have a promo code? Enter it here:</Row>
          <Flex gap={10}>
            <Flex vertical={true}>
              <Input
            ...
            )} onClick={handlePromoCode}>{isPromoCodeActive ? 'Reset' : 'Apply'}</Button>
          </Flex>
        </Flex>
        <Divider className={'bg-accentColor dark:bg-black h-[2px] my-[12px]'} />
        <Row className="text-h3 text-accentColor dark:text-basicColor font-bold md:text-start">Shipping details:</Row>
        <Row className={'text-grayMColor dark:text-black text-[22px] font-bold '}>Name:&nbsp;
          <Row className={'text-black dark:text-accentDarkColor text-[22px] font-bold '}>{userData?.username}</Row>
        </Row>
        <Row className={'text-grayMColor dark:text-black text-[22px] font-bold '}>Address:&nbsp;
          <Row className={'text-black dark:text-accentDarkColor text-[22px] font-bold '}>{userData?.address}</Row>
        </Row>
        <Row className={'text-grayMColor dark:text-black text-[22px] font-bold '}>Tel:&nbsp;
          <Row className={'text-black dark:text-accentDarkColor text-[22px] font-bold '}>{userData?.phone}</Row>
        </Row>
        <Row className={'text-grayMColor dark:text-black text-[22px] font-bold '}>Email:&nbsp;
          <Row className={'text-black dark:text-accentDarkColor text-[22px] font-bold '}>{userData?.email}</Row>
        </Row>
        <Row className={'text-grayMColor dark:text-black text-[22px] font-bold '}>Estimated delivery date:&nbsp;
          <Row className={'text-black dark:text-accentDarkColor text-[22px] font-bold '}>{deliveryDate.toDateString()}</Row>
        </Row>
        <Divider />
        ...
        )} onClick={placeOrder}>Place an order</Button>
      </Flex>
    </Flex>
  );
};
